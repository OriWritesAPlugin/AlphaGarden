...
function getSeedCollectionAsStringWithTemp() { if (on_temporary_collection) {
    return temporary_collection.join(",");
}
else {
    return getSeedCollectionAsString();
} }
;
function getSeedCollectionWithTemp() { if (on_temporary_collection) {
...
function doSeedOnclick(e) {
    ...
    else if (selected_function == "recycle") {
        prep_recycle_seed(seed);
    }
    else if (selected_function == "mutate") {
        prep_mutate_seed(seed);
    }
}
...
function removeSeedFromCollection(seed) {
    ...
    if (index != -1) {
        if (collection == seed) {
            delete localStorage.seed_collection;
        }
        else {
...
function select_seed(seed) {
    ...
    entry.onclick = function () { parent.removeChild(entry); if (parent.childNodes.length == 0) {
        document.getElementById("copy_selection_button").value = "Copy all";
    } };
...
function prep_mutate_seed(seed, force = false) {
    ...
    if (force) {
        if (first_splice.children.length > 0) {
            first_splice.removeChild(first_splice.children[0]);
            first_splice.setAttribute("data-seed", "");
        }
        if (second_splice.children.length > 0) {
            second_splice.removeChild(second_splice.children[0]);
            second_splice.setAttribute("data-seed", "");
        }
    }
    ...
    // If first or both set
        ...
        if (first_splice.getAttribute("data-seed") == seed) {
            return;
...
function update_splice() {
    ...
    set_data("foliage");
    set_data("foliage_palette");
    set_data("feature_palette");
    set_data("accent_palette");
...
function claim_splice() {
    if (!on_temporary_collection) {
        ...
        if (!(gotten && gotten_2)) {
            ...
            if (gotten) {
                collectSeedWithTemp(document.getElementById("first_splice").getAttribute("data-seed"));
            }
            ;
            if (gotten_2) {
                collectSeedWithTemp(document.getElementById("first_splice").getAttribute("data-seed"));
...
function passesFilter(plant_data, colorFilterMode) {
    ...
    if (colorFilterMode == 0) { // any
        ...
        if (applyFilter("palette", plant_data["feature_palette"])) {
            return true;
        }
        if (applyFilter("palette", plant_data["accent_palette"])) {
            return true;
        }
    }
...
function get_seed_collection_sorted_by(collection, sort_key) {
    ...
    if (sort_key == "None" && !needs_filtered) {
        return collection;
    }
    ;
    let parsed_seeds = {};
    ...
    if (needs_filtered) {
        ...
        for (let entry of collection) {
            if (allowed_seeds.has(entry)) {
                filtered_collection.push(entry);
            }
        }
...
function doCollectionPreload() {
    ...
    if (localStorage["last_backup"] == undefined) {
        localStorage["last_backup"] = new Date();
    }
