...
var GardenItemType;
(function (GardenItemType) {
    GardenItemType[GardenItemType["Seed"] = 1] = "Seed";
    GardenItemType[GardenItemType["Catalog"] = 2] = "Catalog";
    GardenItemType[GardenItemType["Overlay"] = 3] = "Overlay";
...
don't overlap.
**/
var GardenItemHeightCategory;
(function (GardenItemHeightCategory) {
    GardenItemHeightCategory[GardenItemHeightCategory["Tiny"] = 1] = "Tiny";
...
class GardenPlacedItem extends GardenItem {
    offset; // fraction of the way across the canvas to put it. .7 = 70%, near the right
...
class GardenOverlayItem extends GardenItem {
    opacity; // Like GardenPlacedItem's offset, is a fraction.
    constructor(identity, type, opacity) {
        super(identity, type);
...
class GardenLayer extends Layer {
    ...
    generateContent() {
        ...
        Object.values(GardenItemHeightCategory).filter(value => !isNaN(Number(value))).forEach(height => {
            this.smart_coords[height] = createSpacedPlacementQueue(this.width, height * 24 - height);
        });
    ...
    generateStarfield(accentColor) {
        ...
        for (let i = 0; i < (pixel_indices * star_ratio); i++) {
            ...
            if (star_roll < 800) {
                ...
                if (star_core > this.canvas.width) {
                    dim_pixel_pos.push(star_core - this.canvas.width);
                }
                if (star_core < pixel_indices - this.canvas.width) {
                    dim_pixel_pos.push(star_core + this.canvas.width);
                }
