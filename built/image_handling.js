...
function load_sprite_from_groundsheet(img, offset) {
    return new Promise(resolve => {
...
async function preload_ground(name, URL, count) {
...
function buildPaletteSwapLookup(old_palettes, new_palettes) {
    ...
    for (let i = 0; i < old_palettes.length; i++) {
        ...
        if (lookup[old_rgb[0]] == undefined) {
            lookup[old_rgb[0]] = {};
        }
        if (lookup[old_rgb[0]][old_rgb[1]] == undefined) {
            lookup[old_rgb[0]][old_rgb[1]] = {};
        }
...
function applyOverlay(stencil_canvas, palette, opacity, full_screen = false) {
...
function tile_along_y(tileCtx, img, yPos, width, xOffset = 0) {
...
async function imageFromPopup(parent, name_of_image, callback, target_size = work_canvas_size / 3 * 2) {
...
async function handleImage(files, name_of_image, preview_img, target_size) {
    if (files.length > 1) {
...
async function resize_for_garden(name_of_image, sourceURL, target_size) {
    const refURL = name_of_image + "_wildcard_data_url";
    const temp_img = await preload_single_image(sourceURL);
    ...
    wildcard_ctx.drawImage(temp_img, (work_canvas_size - target_size) / 2, work_canvas_size - temp_img.naturalHeight * (target_size / max_side), temp_img.naturalWidth * (target_size / max_side), temp_img.naturalHeight * (target_size / max_side));
...
export { refs, replace_color_palette_single_image, preload_single_image, preload_ground, replace_color_palette, applyOverlay, draw_outline_v2, tile_along_y, imageFromPopup, clearCanvas, drawSkyGradient, wildcard_canvases };
