...
class GardenLayerDiv extends LayerDiv {
  ...
  onChangeGardenCallback: () => void;

  constructor(layer: Layer, id: number, onEditCallback: () => void, onChangeGardenCallback: () => void) {
...
class DecorLayerDiv extends LayerDiv {
  constructor(layer: Layer, id: number, onEditCallback: () => void) {
...
class CelestialLayerDiv extends LayerDiv {
  ...
  get_custom_palette(cl: CelestialLayer, callback: () => void) {
...
class LayerManager {
  ...
  scale = 1;
  updateCallback: () => void;
  gardenToggleCallback: () => void;
  activeGardenChangedCallback: () => void;  // Optional callback for alerting other UI elements to active garden changes
  ...
  clearAllButActive() {
    for (const id of Object.keys(this.divToLayerMapper) as string[]) {
  ...
  parseSeedList(seedList: string, callback: () => void) {
